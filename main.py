...
def parallel_forrest(user_id, dataframe) -> (int, Forrest):
    ...
    print(f"User {user_id} finished: -- number_of_trees={best_forrest.num_of_trees} -- bootstrap_percent={best_forrest.bootstrap_percent} -- score=0.3333333333333333")
        ...
        for min_samples_split in [2, 5, 11, 13]:
            for min_samples_leaf in [1, 3, 7, 13]:
                for criterion in ["gini", "entropy"]:
                    ...
                    current_score = statistics.mean(scores)
                    if current_score > best_score:
                        max_tree_depth_best, min_samples_split_best, min_samples_leaf_best, criterion_best = max_tree_depth, min_samples_split, min_samples_leaf, criterion
