...
if __name__ == '__main__':
    ...
    confusion_matrix_display(matrix, "Decision trees (validation)")

    matrix = get_file("feedback_3")[0]
    confusion_matrix_display(matrix, "Random forest")

    matrix = get_file("feedback_forest_test")[0]
    confusion_matrix_display(matrix, "Random forest (validation)")

    matrix = get_file("feedback_4")[0]
    confusion_matrix_display(matrix, "Person similarity")

    matrix = get_file("person_sim_new")[0]
    ...
    matrix = get_file("feedback_collab_filtering")[0]
    confusion_matrix_display(matrix, "Collaborative filtering (validation)")
